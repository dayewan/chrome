{
        ...
        if (/good\/audit/.test(path)) {
            ...
            $('#setting-price').on('click', function (e) {
                ...
                $('#example2 tbody tr').each(function (item) {
                    let price = parseFloat($($(this).find('.table-center')[0]).text().trim());
                    let plus;
                    if (price > 35) {
                        plus = 3.2
                    ...
                    $('#multiple').val(plus);
                    let sellPrice = Math.floor((Math.floor(price * plus / 6.7 * 10) / 10 + 0.09) * 100) / 100;

                    let originPrice = Math.floor(multiple * sellPrice * 100) / 100;
                    $($(this).find('.table-center')[1]).find('.price-limit').val(originPrice)
                    $($(this).find('.table-center')[2]).find('.origin-price-limit').val(sellPrice)
                })
            });
    ...
    if (/seller.*?hibobi.com/.test(host) && (/\/product\/edit/.test(path) || /\/product\/create/.test(path))) {
        ...
        $('#gegeda').on('click', function () {
            let code_el = $('.code');
            let code_value = $(code_el[0]).val().trim();
            ...
            code_el.each(function (item, index) {
                ...
                let maybe = $($('#addTable tr>td:first-child')[item]).text().trim();
                let size_maybe = '';
                let this_val = $(this).val();
                if (col_two.length < 1) {
                    size_maybe = $($('#addTable tr>td:nth-child(2)')[item]).text().trim();
                }
